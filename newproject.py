...
#----------------------------------------FUNCTIONS----------------------------------------------------------------------------
def iExit():
    qExit = messagebox.askyesno("EDMS","Do You Want To Exit Program")
    if qExit > 0:
        root.destroy()
        return
...
def create_table():
    ...
    entryTname = Entry(f2 ,textvariable=tname ,font=("arial",13,"bold"), bg = "bisque3",width = 50).grid(row=1,column=3)
...
def data_entry():
    ...
    entryTname1 = Entry(f2 ,textvariable=tname1 ,font=("arial",13,"bold"),bg = "bisque3", width = 50).grid(row=1,column=3)
...
def print_data():
    ...
    def official_print_data():
        ...
        data = c.fetchall()
        for k in data:
            labelPrint = Label(f2 ,text=k,background = '#EBFFFF',width=50,font=("arial",25,"bold")).pack(side=RIGHT)


    labelTname2 = Label(f2 ,text="Table Name:",font=("arial",10,"bold")).grid(row=1,column=0)
    entryTname2 = Entry(f2 ,textvariable=tname3 ,font=("arial",13,"bold"), bg = "bisque3",width = 50).grid(row=1,column=3)
...
def delete_table():
    ...
    entryTname3 = Entry(f2 ,textvariable=tname4 ,font=("arial",13,"bold"),bg = "bisque3", width = 50).grid(row=1,column=3)
...
def access_db():
    ...
    def login_base():
        ...
        if((username == "admin" and password == 4087) or (username == "ADMIN" and password == 4087)):
            ...
            data = obj.readlines()
            for k in data:
                labelFILE = Label(f2 ,text=k,width = 50,bg = "bisque3",font=("arial",25,"bold")).pack(side=TOP)
            obj.close()
    ...
    entryUNAME = Entry(f2 ,textvariable=UNAME ,font=("arial",13,"bold"), bg = "bisque3",width = 50).grid(row=1,column=3)
    
    labelPASS  = Label(f2 ,text="Password (INT):",font=("arial",10,"bold")).grid(row=3,column=0)
    entryPASS = Entry(f2 ,textvariable=UPASS,show="*" ,font=("arial",13,"bold"),bg = "bisque3", width = 50).grid(row=3,column=3)
...
f1.pack(side=LEFT)
f2 = Frame(root , width=600, height=600 , bd=8, relief="raise")
f2.pack(side=RIGHT)

f1a = Frame(f1 , width = 600,height = 200,bd=20 , relief="raise")
f1a.pack(side=TOP)
f1b = Frame(root , width = 800 ,height = 300)
f1b.pack(side=BOTTOM)
labelf1b = Label(f1b , font=("Times new roman",12,"bold"),text=":::::::STATUS ::::::::").pack(side=TOP)


labelinfo = Label(frame1 , font=("Times New Roman",35,"bold"),text="                EMPLOYEE DETAILS MANAGEMENT SYSTEM                ",fg="blue",bg = "bisque3")
labelinfo.grid(row=0,column=0)

button1 = Button(f1a ,padx=10,pady=10, text = "  CREATE AN EMPLOYEE  ",font=("Times New Roman",13,"bold"),bd=5,fg="firebrick3",bg = "gray15",command= create_table).grid(row=0,column=0)

button2 = Button(f1a ,padx=10,pady=10, text = " ENTER EDATA  ",font=("Times New Roman",13,"bold"),fg="dark green",bd=10,bg = "gray15",command= data_entry).grid(row=1,column=0)

button3 = Button(f1a ,padx=10,pady=10, text = " DISPLAY  EDETAILS",font=("Times New Roman",13,"bold"),bd=10,fg="gold4",bg = "gray15",command= print_data).grid(row=2,column=0)

button4 = Button(f1a ,padx=10,pady=10, text = "  DELETE AN EMPLOYEE ",font=("Times New Roman",13,"bold"),bd=10,fg="Dark orchid1",bg = "gray15",command = delete_table).grid(row=3,column=0)

button5 = Button(f1a ,padx=10,pady=10, text = " ACCESS DATABASE ",font=("Times New Roman",13,"bold"),bd=10 ,fg="cyan" ,bg = "gray15",command = access_db).grid(row=4,column=0)

button1 = Button(f1a ,padx=10,pady=10, text = " EXIT PROGRAM ",font=("Times New Roman",13,"bold"),bd=10,fg="coral",bg = "gray15",command=iExit).grid(row=5,column=0)
